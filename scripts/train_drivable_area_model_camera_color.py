...
DATASET_ROOT = Path("outputs/dataset_camera_color")
INPUT_DIR = DATASET_ROOT / "inputs"
LABEL_DIR = DATASET_ROOT / "labels_drivable"
METADATA_PATH = DATASET_ROOT / "metadata.json"

OUTPUT_DIR = Path("outputs/model_drivable_camera_color")
...
class BevDrivableCameraColorDataset(Dataset):
...
def save_prediction_examples(model, val_dataset, epoch_label: str):
    ...
    with torch.no_grad():
        for row, index in enumerate(selected_indices):
            ...
            axes[row, 4].set_title("Prediction mask")
            axes[row, 4].axis("off")

    plt.suptitle(f"LiDAR plus camera color prediction examples, {epoch_label}")
    plt.tight_layout()

    output_path = FIGURE_DIR / f"drivable_area_camera_color_prediction_examples_{epoch_label}.png"
...
def main():
    ...
    train_dataset = BevDrivableCameraColorDataset(train_samples, augment=True)
    val_dataset = BevDrivableCameraColorDataset(val_samples, augment=False)
    ...
    best_val_iou = -1.0
    best_model_path = OUTPUT_DIR / "best_drivable_area_camera_color_model.pt"
    final_model_path = OUTPUT_DIR / "final_drivable_area_camera_color_model.pt"
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("LiDAR plus camera color drivable area training loss")
    plt.legend()
    plt.grid(True, alpha=0.3)

    loss_curve_path = FIGURE_DIR / "drivable_area_camera_color_training_loss.png"
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Metric")
    plt.title("LiDAR plus camera color validation metrics")
    plt.legend()
    plt.grid(True, alpha=0.3)

    metrics_curve_path = FIGURE_DIR / "drivable_area_camera_color_validation_metrics.png"
